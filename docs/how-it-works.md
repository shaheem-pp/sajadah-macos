...
- **`AladhanAPI`** fetches one calendar month per request from `/v1/calendar/{year}/{month}` with `iso8601=true`, so every timing arrives as a fully-offset instant and DST needs no special handling. By default the request carries no `method`: Aladhan then computes with the authority nearest the coordinates (ISNA in North America, MWL in Europe, and so on) and names it in `meta.method`, which is cached alongside the timings so the footer can say what "Automatic" resolved to.
